...
func (r *Room) Entry(ctx context.Context) (*pbcommon.Response, error) {
...
// Join room
func (r *Room) Join(ctx context.Context) (*pbcommon.Response, error) {
	fakeUID := uuid.New().String() // just use s.ID as uid !!!
	s := pitaya.GetSessionFromCtx(ctx)
	err := s.Bind(ctx, fakeUID) // binding session uid
	if err != nil {
...
// SendRPC sends rpc
func (r *Room) SendRPC(ctx context.Context, msg []byte) (*pbcommon.Response, error) {
